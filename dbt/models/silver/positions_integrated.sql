...
--   A7  (Mixed grain)           — Emerald is lot-level; Topaz/Ruby are position-level
...
-- A7: record_id is a composite position key — position-level grain
-- A9: cost_basis = custodian position cost
-- A2: price = custodian EOD baseline (authoritative, but not distinguished from other sources)

topaz_norm AS (
    SELECT
        CONCAT('POS-', ACCT_NUM, '-', SEC_CUSIP)            AS record_id,   -- A7: position ID
        ...
        UNITS                                                AS quantity,       -- position-level
        MKT_PRC                                              AS price,          -- A2: custodian EOD
        MKT_VAL                                              AS market_value,
        COST_BASIS                                           AS cost_basis,     -- A9: custodian cost
...
-- A9: cost_basis = specific lot total cost (Emerald OMS tracks per-trade cost)
-- A2: price = PM evaluated (custodian EOD ±0.3% variance baked in)

emerald_norm AS (
    SELECT
        LOT_ID                                               AS record_id,   -- A7: lot-level ID
        ...
        QUANTITY                                             AS quantity,       -- A7: lot-level — trap for SUM
        UNIT_PRICE                                           AS price,          -- A2: PM evaluated
        MARKET_VALUE                                         AS market_value,
        LOT_COST_BASIS                                       AS cost_basis,     -- A9: specific lot cost
